...
- **Java 25 is the minimum language level**, not a ceiling. Compile with
  ...
  any project that must pin an older level and explain why (see §6). One cap applies to native
...
- **Build for both JVM and native.** Keep a `native` Maven profile so the project can produce a
  ...
  see §2 — so a project with a native profile does not raise the language level above 25.
...
- **Tools are CDI beans.** Expose actions to a model with `@Tool` methods on `@ApplicationScoped`
  beans, wired via `@RegisterAiService(tools = …)` or `@ToolBox` — never hand-rolled JSON function
  dispatch. Tool methods doing I/O follow the §2 virtual-thread rules.
