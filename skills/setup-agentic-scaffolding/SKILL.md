...
- **context7** — command `npx`, args `-y @upstash/context7-mcp@4.1.0`. The optional
...
|---|---|---|---|
| Claude Code | `claude mcp add -s user quarkus-agent -- jbang --java 21+ io.quarkus:quarkus-agent-mcp:1.2.6:runner` · `claude mcp add -s user context7 -- npx -y @upstash/context7-mcp@4.1.0` | `claude mcp list` | No — restart |
| Codex CLI | `codex mcp add quarkus-agent -- jbang --java 21+ io.quarkus:quarkus-agent-mcp:1.2.6:runner` · `codex mcp add context7 -- npx -y @upstash/context7-mcp@4.1.0` | `codex mcp list` | No — restart; sandbox may block network |
| Gemini CLI | `gemini mcp add -s project quarkus-agent jbang --java 21+ io.quarkus:quarkus-agent-mcp:1.2.6:runner` · `gemini mcp add -s project context7 npx -y @upstash/context7-mcp@4.1.0` (project scope — `gemini mcp add` defaults to `--scope project`; replace `-s project` with `-s user` to register them for every project) — **or** install this repo's Gemini extension, which already declares both servers | `gemini mcp list` | No — restart |
...
```json
{
  "mcpServers": {
    "quarkus-agent": { "command": "jbang", "args": ["--java", "21+", "io.quarkus:quarkus-agent-mcp:1.2.6:runner"] },
    "context7":      { "command": "npx",   "args": ["-y", "@upstash/context7-mcp@4.1.0"] }
  }
}
```
...
`npx -y @upstash/context7-mcp@4.1.0` for `context7` — and treat a mismatch on either server as a
