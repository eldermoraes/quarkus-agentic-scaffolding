...
- **quarkus-agent** — command `jbang`, args `--java 21+ io.quarkus:quarkus-agent-mcp:1.2.8:runner`
...
|---|---|---|---|
| Claude Code | `claude mcp add -s user quarkus-agent -- jbang --java 21+ io.quarkus:quarkus-agent-mcp:1.2.8:runner` · `claude mcp add -s user context7 -- npx -y @upstash/context7-mcp@4.1.1` | `claude mcp list` | No — restart |
| Codex CLI | `codex mcp add quarkus-agent -- jbang --java 21+ io.quarkus:quarkus-agent-mcp:1.2.8:runner` · `codex mcp add context7 -- npx -y @upstash/context7-mcp@4.1.1` | `codex mcp list` | No — restart; sandbox may block network |
| Gemini CLI | `gemini mcp add -s project quarkus-agent jbang --java 21+ io.quarkus:quarkus-agent-mcp:1.2.8:runner` · `gemini mcp add -s project context7 npx -y @upstash/context7-mcp@4.1.1` (project scope — `gemini mcp add` defaults to `--scope project`; replace `-s project` with `-s user` to register them for every project) — **or** install this repo's Gemini extension, which already declares both servers | `gemini mcp list` | No — restart |
...
| GitHub Copilot CLI | `copilot mcp add quarkus-agent -- jbang --java 21+ io.quarkus:quarkus-agent-mcp:1.2.8:runner` · `copilot mcp add context7 -- npx -y @upstash/context7-mcp@4.1.1` | `copilot mcp list` | **Yes** — live immediately |
| opencode | Write `opencode.json` `mcp` key with both servers | `/mcp` in session | **Yes** — hot reload |
| Bob (D3; read §5.1 first) | `bob mcp add -s global quarkus-agent jbang -- --java 21+ io.quarkus:quarkus-agent-mcp:1.2.8:runner` · `bob mcp add -s global context7 npx -- -y @upstash/context7-mcp@4.1.1` — the `--` is mandatory, and `-s global` is machine-wide: state that to the user and offer `-s workspace` to stack-mixers (both §5.1) | `bob mcp list` shows both, `stdio`, `global` (or `workspace`) | **Yes** — Bob restarts changed servers |
...
pinned string — `jbang --java 21+ io.quarkus:quarkus-agent-mcp:1.2.8:runner` for `quarkus-agent`,
