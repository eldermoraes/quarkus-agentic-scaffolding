...
- **`add` never updates an existing entry — `add-json` does.** On a name that is already registered,
  ...
  `bob mcp add-json -s <scope> quarkus-agent '{"command":"jbang","args":["--java","21+","io.quarkus:quarkus-agent-mcp:1.2.8:runner"]}'`
  · `bob mcp add-json -s <scope> context7 '{"command":"npx","args":["-y","@upstash/context7-mcp@4.1.1"]}'`.
